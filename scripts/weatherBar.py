...
procGrep = subprocess.Popen(
    ["grep", "color9"], stdin=procCat.stdout, stdout=subprocess.PIPE)
...
try:
    ...
    try:
        tempLocal = '<span ' + markup + ' >' + \
            str(jsonLocalData['temp']) + '</span>'
        humLocal = '<span ' + markup + ' >' + \
            str(jsonLocalData['hum']) + '</span>'
    except KeyError, e:
